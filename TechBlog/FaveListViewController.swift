...
class FaveListViewController: UITableViewController, CellDelegate2 {
    ...
    private func loadData() {
            ...
            let faveRef = db.collection("users").document("\(user.uid)").collection("fave")
    ...
    func didUnTapButton2(cell: CustomCell2) {
        ...
        let indexPath = tableView.indexPath(for: cell)?.row
        if let user = Auth.auth().currentUser {
            db.collection("users").document("\(user.uid)").collection("fave").document("\(favorites[indexPath!].title)").delete() { err in
            if let err = err {
